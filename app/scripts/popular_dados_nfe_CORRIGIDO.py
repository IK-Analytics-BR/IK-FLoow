...
VERSÃO CORRIGIDA - Usa os nomes de campos corretos
"""
...
def popular_empresas(conn):
    ...
    insert_query = """
        INSERT INTO empresas (
            ...
            bairro, cidade, estado, pais, ativo, created_at
        ) VALUES (
            %s, %s, %s, %s,
            %s, %s, %s, %s, %s,
            %s, %s, %s, %s, %s, %s
...
def popular_clientes(conn):
    ...
    insert_query = """
        INSERT INTO customers (
            name, razao_social, cnpj, cnpj_basico, ie,
            phone, email, cep, address, number, complement,
            neighborhood, city, state, active, origem_cadastro, created_at
        ) VALUES (
            %s, %s, %s, %s, %s,
            %s, %s, %s, %s, %s, %s,
            %s, %s, %s, %s, %s, %s
...
def verificar_campos_produtos(conn):
    ...
    campos_necessarios = [
        ("supplier_code", "VARCHAR(60)", "Código do produto do fornecedor", "name"),
        ("barcode", "VARCHAR(14)", "Código de barras EAN", "supplier_code"),
        ("internal_code", "VARCHAR(60)", "Código interno do produto", "barcode")
    ]
...
def popular_produtos(conn):
    ...
    # 1. Limpar tabela
    print("\n[1/4] Limpando tabela de produtos...")
    cursor.execute("SET FOREIGN_KEY_CHECKS = 0")
    ...
    # 2. Buscar produtos únicos com médias de alíquotas
    print("\n[2/4] Buscando produtos únicos das NF-e...")
    query = """
        SELECT 
            codigo_produto,
            codigo_ean,
            ...
            AVG(COALESCE(icms_aliquota, 0)) as icms_media,
            AVG(COALESCE(ipi_aliquota, 0)) as ipi_media,
            AVG(COALESCE(pis_aliquota, 0)) as pis_media,
            AVG(COALESCE(cofins_aliquota, 0)) as cofins_media,
    ...
    # 3. Buscar CST e Origem mais comuns para cada produto
    print("\n[3/4] Buscando informações fiscais...")
    
    # 4. Inserir produtos
    print("\n[4/4] Inserindo produtos...")
    insert_query = """
        INSERT INTO products (
            name, supplier_code, barcode, description,
            ncm, cest, cfop_in, cfop_out, unit_measure,
            price, cost_price, icms_rate, ipi_rate, pis_rate, cofins_rate,
            cst_csosn, origin, stock_quantity, min_stock, category,
            active, imported, created_at
        ) VALUES (
            %s, %s, %s, %s,
            %s, %s, %s, %s, %s,
            %s, %s, %s, %s, %s, %s,
    ...
    for produto in produtos:
        try:
            ...
            # Buscar CST mais comum para este produto
            cursor.execute("""
                SELECT icms_cst 
                FROM nfe_staging_itens 
                WHERE codigo_produto = %s 
                AND icms_cst IS NOT NULL
                GROUP BY icms_cst 
                ORDER BY COUNT(*) DESC 
                LIMIT 1
            """, (produto['codigo_produto'],))
            
            cst_result = cursor.fetchone()
            cst_csosn = cst_result['icms_cst'] if cst_result else None
            
            # Buscar origem mais comum
            cursor.execute("""
                SELECT icms_origem 
                FROM nfe_staging_itens 
                WHERE codigo_produto = %s 
                AND icms_origem IS NOT NULL
                GROUP BY icms_origem 
                ORDER BY COUNT(*) DESC 
                LIMIT 1
            """, (produto['codigo_produto'],))
            
            origin_result = cursor.fetchone()
            origin = origin_result['icms_origem'] if origin_result else None
            ...
            cursor.execute(insert_query, (
                ...
                preco,
                preco,
                round(float(produto['icms_media']), 2),
                round(float(produto['ipi_media']), 2),
                round(float(produto['pis_media']), 4),
                round(float(produto['cofins_media']), 4),
                cst_csosn,
                origin,
                0,  # stock inicial
                5,  # min_stock
                'Importado NFe',  # category
                True,
                True,
                datetime.now()
            ))
    ...
    # Adicionar índices
    print("\n[EXTRA] Criando índices...")
    try:
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_supplier_code ON products(supplier_code)")
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_internal_code ON products(internal_code)")
        conn.commit()
...
def exibir_estatisticas(conn):
    ...
    cursor.execute("""
        SELECT 
            SUM(CASE WHEN icms_rate > 0 THEN 1 ELSE 0 END) as com_icms,
            SUM(CASE WHEN ipi_rate > 0 THEN 1 ELSE 0 END) as com_ipi,
            SUM(CASE WHEN pis_rate > 0 THEN 1 ELSE 0 END) as com_pis,
            SUM(CASE WHEN cofins_rate > 0 THEN 1 ELSE 0 END) as com_cofins
        FROM products
    """)
    
    fiscal = cursor.fetchone()
    print(f"   Com ICMS: {fiscal['com_icms']}")
    print(f"   Com IPI: {fiscal['com_ipi']}")
    print(f"   Com PIS: {fiscal['com_pis']}")
    print(f"   Com COFINS: {fiscal['com_cofins']}")
...
def main():
    """Função principal"""
    print("\n" + "="*80)
    print("POPULAR DADOS DE NF-e PARA TABELAS PRINCIPAIS (VERSÃO CORRIGIDA)")
    print("="*80)
